...
namespace ImGui
{
	...
	int Bezier(const char *label, float P[5]) {
		...
		// prepare canvas
		const float avail = GetContentRegionAvail()[0];
		...
		// eval curve
		// Swap eval curve to from 1,1 to 0,0 instead of 0,0 to 1,1
		ImVec2 Q[4] = { { 0, 0 }, { P[0], P[1] }, { P[2], P[3] }, { 1, 1 } };
...
//		}
	...
	int MyBezier(const char *label, ImVec2 (&Q)[4], const int channel = 0)
	{
		// visuals
		enum { SMOOTHNESS = 64 }; // curve smoothness: the higher number of segments, the smoother curve
		enum { CURVE_WIDTH = 4 }; // main curved line width
...
//			return changed;
		...
		// Curve
//		ImVec2 &Q[4] = { { 0, 0 }, { P[0], P[1] }, { P[2], P[3] }, { 1, 1 } };
		float P[4] = {Q[1][0], Q[1][1], Q[2][0], Q[2][1]};
		ImVec2 results[SMOOTHNESS + 1];
		bezier_table<SMOOTHNESS>(Q, results);
		// control points: 2 lines and 2 circles
		{
...
//		float P[4] = {Q[1][0], Q[1][1], Q[2][0], Q[2][1]};
		...
		// Set colour of circle based on current channel bezier is editing
		DrawList->AddCircleFilled(p1, GRAB_RADIUS, ImColor(white));
		DrawList->AddCircleFilled(p1, GRAB_RADIUS - GRAB_BORDER, ImColor(pink));
		DrawList->AddCircleFilled(p2, GRAB_RADIUS, ImColor(white));
		DrawList->AddCircleFilled(p2, GRAB_RADIUS - GRAB_BORDER, ImColor(cyan));
